...
# path
folder_path = 'C:/Users/Chen/Desktop/gala_scrapy/crawler_temp/'
...
category_list = ['game_action', 'game_adventure', 'game_arcade', 'game_board', 'game_card', 'game_casino',
...
for category in category_list:
    ...
    for app in data:
        new_df = pd.DataFrame({"rank": app["rank"], "name": app["name"], "app_id": app["app_id"], "category": category, 'country': country, "channel": channel,  "rating": app["rating"], "rating_count": app["rating_count"]}, index=[0])
        df = pd.concat([df, new_df], ignore_index=True)
    df = df.drop(0)
    df.to_csv(folder_path + "/" + country + "/" + country + "_" + channel + "_" + category + "_" + today_str + '.csv')
    print(df)
